...
class tagged_base {
public:
  tagged_base(T const &x) : value_(x) {}
  tagged_base(T &x) : value_(x) {}
...
template<class T>
bool operator==(tagged_base<T> const &lhs, tagged_base<T> const &rhs) {
...
template<class T>
std::ostream &operator<<(std::ostream &os, tagged_base<T> const &v) {
  return os << v.get();
}
