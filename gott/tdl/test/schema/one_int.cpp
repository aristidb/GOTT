...
struct schema_one_int : schema_basic {
  schema_one_int() 
  : schema_basic(
      rule_one("document", 
         rule_one("list",
           rule("node",
             rule_attr("el",
                repatcher = int_r(), 
                outer = slotcfg(slotcfg::one)))))) {}
};

typedef schema_one_int group_fixture_t;
}
...
XTEST((n, "many") (gf, 1) (gen, (testsoon::range_generator<int>)(2)(5))) {
  std::ostringstream w;
