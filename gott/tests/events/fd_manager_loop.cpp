...
public:
  ...
  void test_select_loop_read() {
    int pipes[2];
    if(pipe(pipes) == -1) {
      std::cout << "ERROR: Test Failed because of internal issues\n";
      return;
    }
	...
	(pipes[0], fd_manager::read, boost::bind
	 (fd_manager_loop_test::read_event, loop.get(), pipes[0], _1));
      //      boost::bind(&fd_manager_loop_test::read_event, loop.get(), pipes[0], _1)(0);
      loop->run();
      wait(0x0);
    }
  }
