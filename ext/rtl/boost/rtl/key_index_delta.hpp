...
#ifndef BOOST_RTL_KEY_INDEX_DELTA_HPP_INCLUDED
#define BOOST_RTL_KEY_INDEX_DELTA_HPP_INCLUDED
...
namespace boost { namespace rel {
	...
	template<class R> 
    struct traits_impl<R, key_index_tag>
	{
        typedef typename basic_key_index_t<
            typename traits<typename R::arg_type>::plus_type, 
            typename R::sort_list, 
            typename R::fast_list, 
            typename R::strategy_type
        >::type plus_type;
	...
	template<class R>
    typename traits<R>::delta_type delta(const R& idx, const transaction& tr, key_index_tag)
	{
...
#endif//BOOST_RTL_KEY_INDEX_DELTA_HPP_INCLUDED
